...
sgx_status_t verify_secret_data(sgx_ra_context_t context, uint8_t* secret, uint32_t secret_size, uint8_t gcm_mac[16], uint32_t max_verification_length, uint8_t salt[32], uint8_t salt_mac[16], uint32_t* id);
sgx_status_t add_hash_data(uint32_t id, sgx_ra_context_t context, uint8_t* hashdata, size_t hash_size, uint8_t mac[16]);
sgx_status_t get_result_size(uint32_t id, size_t* len);
