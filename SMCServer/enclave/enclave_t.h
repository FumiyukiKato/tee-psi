...
sgx_status_t remote_attestation_mock(uint8_t token[32], uint8_t sk[32]);
...
sgx_status_t judge_contact(uint8_t session_token[32], uint8_t* encrypted_geohash_data, size_t geo_data_size, uint8_t* encrypted_timestamp_data, size_t time_data_size, uint8_t risk_level[24], uint8_t* result, size_t result_size);
