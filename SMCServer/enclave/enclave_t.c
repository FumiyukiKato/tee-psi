...
static sgx_status_t SGX_CDECL sgx_remote_attestation_mock(void* pms)
{
	CHECK_REF_POINTER(pms, sizeof(ms_remote_attestation_mock_t));
	//
	// fence after pointer checks
	//
	sgx_lfence();
	ms_remote_attestation_mock_t* ms = SGX_CAST(ms_remote_attestation_mock_t*, pms);
	sgx_status_t status = SGX_SUCCESS;
	...
	if (_tmp_token != NULL && _len_token != 0) {
		...
		if ((_in_token = (uint8_t*)malloc(_len_token)) == NULL) {
			status = SGX_ERROR_OUT_OF_MEMORY;
			goto err;
		}

		memset((void*)_in_token, 0, _len_token);
	}
	if (_tmp_sk != NULL && _len_sk != 0) {
		if ( _len_sk % sizeof(*_tmp_sk) != 0)
		{
			status = SGX_ERROR_INVALID_PARAMETER;
			goto err;
		}
		if ((_in_sk = (uint8_t*)malloc(_len_sk)) == NULL) {
			status = SGX_ERROR_OUT_OF_MEMORY;
			goto err;
		}
	...
	ms->ms_retval = remote_attestation_mock(_in_token, _in_sk);
	if (_in_token) {
		if (memcpy_s(_tmp_token, _len_token, _in_token, _len_token)) {
			status = SGX_ERROR_UNEXPECTED;
			goto err;
		}
	}
	if (_in_sk) {
		if (memcpy_s(_tmp_sk, _len_sk, _in_sk, _len_sk)) {
			status = SGX_ERROR_UNEXPECTED;
			goto err;
		}
	}
...
static sgx_status_t SGX_CDECL sgx_judge_contact(void* pms)
{
	CHECK_REF_POINTER(pms, sizeof(ms_judge_contact_t));
	//
	// fence after pointer checks
	//
	sgx_lfence();
	ms_judge_contact_t* ms = SGX_CAST(ms_judge_contact_t*, pms);
	sgx_status_t status = SGX_SUCCESS;
	...
	size_t _len_result = 27 * sizeof(uint8_t);
	...
	size_t _len_signature = 64 * sizeof(uint8_t);
	uint8_t* _in_signature = NULL;
	uint8_t* _tmp_user_id = ms->ms_user_id;
	size_t _len_user_id = 16 * sizeof(uint8_t);
	...
	ms->ms_retval = judge_contact(_in_session_token, _in_encrypted_secret_key, _in_secret_key_gcm_tag, _in_encrypted_history_data, _tmp_toal_size, _in_gcm_tag, _tmp_gcm_tag_total_size, _in_size_list, _tmp_data_num, _in_result, _in_result_mac, _in_signature, _in_user_id);
	if (_in_result) {
...
static sgx_status_t SGX_CDECL sgx_get_public_key(void* pms)
{
	...
	size_t _len_public_key = 64 * sizeof(uint8_t);
	...
	ms->ms_retval = get_public_key(_in_session_token, _in_public_key, _in_gcm_tag);
	if (_in_public_key) {
	...
	if (_in_gcm_tag) {
		if (memcpy_s(_tmp_gcm_tag, _len_gcm_tag, _in_gcm_tag, _len_gcm_tag)) {
			status = SGX_ERROR_UNEXPECTED;
			goto err;
		}
	}
